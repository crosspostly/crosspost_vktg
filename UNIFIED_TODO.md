# 📋 Unified TODO — VK→Telegram Crossposter (Server + Client + Fixes + Code Samples)

Обновлено: 2025-11-03 04:31 MSK
Назначение: ЕДИНЫЙ файл задач. Все прочие todo/readme/to_fix — удалить. Рабочая истина здесь.

---

## 🚨 КРИТИЧНЫЕ БАГИ (ИСПРАВИТЬ НЕМЕДЛЕННО)

### 🐛 VK URL Parsing Bug - "Посты из левой группы"
**Проблема:** https://vk.com/club96798355?from=groups → посты идут не из той группы!

**CLIENT.GS исправления:**
- [ ] В `checkNewPosts()`: передавать `vkGroupId` вместо `vkGroupUrl` в `getVkPosts()`
- [ ] В `getVkPosts()`: принимать и использовать `vk_group_id` вместо `vk_group_url`
- [ ] Добавить детальное логирование ID на каждом шаге
- [ ] Убедиться что `extractVkGroupId()` корректно извлекает ID из URL с параметрами

**SERVER.GS исправления:**
- [ ] В `handleGetVkPosts()`: принимать `vk_group_id` вместо `vk_group_url` 
- [ ] Валидация что получен корректный owner_id для VK API
- [ ] Логировать реальный API запрос к VK для диагностики
- [ ] Обработка edge cases: screen_name vs numeric ID

**Диагностика:**
```javascript
// CLIENT: добавить в checkNewPosts()
logEvent("DEBUG", "vk_id_extraction", "client", 
         `URL: ${binding.vkGroupUrl} → ID: ${vkGroupId}`);

// SERVER: добавить в handleGetVkPosts()  
logEvent("DEBUG", "vk_api_request", "server",
         `VK API URL: https://api.vk.com/method/wall.get?owner_id=${vk_group_id}`);
```

---

## 🔧 SERVER (server.gs)

### A. Рефакторинг (от простого к сложному)
1) API-обёртка и JSON парсер
- [ ] makeApiRequest(service, url, options)
- [ ] parseJsonSafe(text)
- Внедрить в: Telegram sendMessage/sendMediaGroup и VK wall.get/video.get

2) Валидация payload/лицензии/ответов
- [ ] validatePayload(payload, requiredFields)
- [ ] validateLicenseOrThrow(licenseKey)
- [ ] success(data, meta?) / fail(message, code?, meta?) + jsonResponse

3) Caption split централизованно
- [ ] ensureCaptionOrSplit(token, chatId, mediaUrls, caption)
- Сократить sendVkPostToTelegram, убрать дубли

4) Утилиты Sheets (минимум)
- [ ] getRows(name), findRow(name, predicate), appendRow(name, obj)

5) Вынести утилиты в файлы (при желании)
- api-utils.gs, validation-utils.gs, sheets-utils.gs, html-builder.gs, config-manager.gs

### B. Поведение публикации/медиа
6) Публикация реального поста без «теста»
- [ ] Если vk_post не передан в handleSendPost — сервер сам берёт 1 или N неприкреплённых постов из VK по настройке связки

7) Медиа-группы строго одной пачкой
- [ ] Собрать фото в исходном порядке → один sendMediaGroup (до 10)
- [ ] Видео/аудио/доки — отдельным сообщением (список ссылок)

### C. Ссылки и форматирование
8) VK → Telegram Markdown ссылки (НЕ ссылка на пост)
- [ ] [screen_name|Title] → [Title](https://vk.com/screen_name)
- [ ] [club/public/id123|Title] поддержаны
- [ ] [https://example.com|Текст] → [Текст](https://example.com)
- [ ] vk.com/wall-… допускается как отдельный линк, но не подменяет Title-ссылки
- [ ] vk.cc/… оставлять как есть (внешний сайт возможен)
- [ ] Экранировать спецсимволы Markdown в Title
- Тест-кейсы — см. ниже

9) Опции форматирования по флагам
- [ ] Первая строка жирным (on/off)
- [ ] Слова ВЕРХНИМ РЕГИСТРОМ — жирным (on/off)

10) Названия VK/TG — стабильная выдача и кеш
- [x] ✅ getVkGroupName/getTelegramChatName + кеш
- [x] ✅ Информативные логи при ошибках

11) Enhanced Error Logging
- [ ] logApiCall (категории VK_API/TG_API/SYSTEM/USER, http-status, body)
- [ ] Авто-ротация логов

12) Comprehensive Error Handling
- [ ] retryApiCall(backoff)
- [ ] Строгая валидация входных параметров

### D. 🚀 НОВОЕ: Оптимизация производительности
13) Кеширование на сервере
- [x] ✅ PropertiesService кеш для VK/TG названий
- [ ] Батчевые операции для получения названий (handleBatchGetNames)
- [ ] Фоновое обновление кеша по расписанию

14) Оптимизация API запросов
- [ ] Уменьшить таймауты: FAST (8с), MEDIUM (15с), SLOW (30с)
- [ ] Fallback отображение при отсутствии в кеше
- [ ] Ограничение одновременных запросов

### F. 🎯 КРИТИЧНО: Client-Server синхронизация
15) Поддержка новых полей связок
- [ ] **В handleAddBinding**: принимать binding_name, binding_description
- [ ] **В handleEditBinding**: обновление binding_name, binding_description
- [ ] **В Bindings листе**: добавить колонки "Binding Name", "Binding Description"
- [ ] **В getUserBindingsWithNames**: возвращать название и описание связки
- [ ] **Миграция данных**: автосоздание новых колонок при первом запуске
- [ ] **В initializeServer**: обновить структуру Bindings листа

16) Обратная совместимость
- [ ] **Fallback логика**: если новые поля пустые → использовать VK Group Name
- [ ] **Валидация**: обязательность binding_name при создании связки
- [ ] **UI синхронизация**: клиент должен корректно отображать все поля с сервера

17) handlePublishLastPost обработчик
- [ ] **Новый case в doPost()**: "publish_last_post" → handlePublishLastPost()
- [ ] **Реализация функции**: получить последний пост из VK + отправить в TG
- [ ] **Логирование**: детальные логи публикации без передачи vk_post
- [ ] **Интеграция с форматированием**: использовать настройки связки

---

## 🖥️ CLIENT (client.gs)

1) «Опубликовать последний пост» вместо «Тест»
- [x] ✅ UI: заменить кнопку на «🧪 Тест» (реализовано)
- [x] ✅ Функция publishLastPost() добавлена корректно
- [ ] Вызов event "send_post" без vk_post → сервер публикует по настройке

2) Первая синхронизация: 1 или N постов
- [x] ✅ В UI: select с опциями синхронизации (1, 3, 5, 10 постов)
- [ ] Передача sync_recent_enabled, sync_recent_count в payload
- [ ] Сохранение на сервере в Bindings
- [ ] Логика инициальной синхронизации при создании связки

3) Published-листы по названию группы
- [ ] getOrCreatePublishedPostsSheet(vkGroupId, vkGroupName) - новый параметр
- [ ] markPostAsSent() - передача названий групп/чатов
- [ ] Новые колонки: Post Preview, TG Chat Name
- [ ] Формат даты DD.MM.YYYY, HH:mm (RU локаль)
- [ ] Миграция старых листов Published_-123456 → Published_GroupName

4) Форматирование — опции в связке
- [x] ✅ Чекбоксы: «Первая строка жирным», «ВСЕ ЗАГЛАВНЫЕ — жирным»
- [x] ✅ Передача format_settings в payload
- [x] ✅ Сохранение настроек в modal форме

5) Массовый переключатель
- [x] ✅ Чекбокс "Выключить все магазины" в UI
- [x] ✅ toggleAllStores() с сохранением в глобальные настройки
- [x] ✅ Интеграция с handleSetGlobalSetting

6) Активация разрешений ScriptApp (без appsscript.json)
- [x] ✅ setupTrigger() с детальными сообщениями
- [x] ✅ Кнопка «⏱️ Авто-проверка» в UI
- [x] ✅ Проверка существующих триггеров перед созданием

7) UI имён и статусов  
- [x] ✅ Показ названий связок в карточках (bindingName + vkGroupName)
- [x] ✅ Fallback на URL/chat_id если названия отсутствуют
- [x] ✅ Статусы активных/приостановленных связок
- [ ] Warning индикаторы для проблем с токенами/правами

### G. 🎛️ НОВОЕ: UI/UX улучшения
10) Sidebar и collapse функциональность
- [x] ✅ Кнопка сворачивания панели в заголовке
- [x] ✅ Mini-панель со статусом при сворачивании  
- [x] ✅ CSS классы .collapsed и .mini-controls
- [ ] **ВАЖНО**: изменить openMainPanel() → openSidebar() с showSidebar()

11) Enhanced binding management
- [x] ✅ Поля binding_name и binding_description в modal формах
- [x] ✅ Валидация обязательности названия связки
- [x] ✅ Отображение названий в карточках связок
- [ ] **КРИТИЧНО**: server.gs должен поддерживать новые поля

### H. 🚀 Оптимизация UI производительности  
12) Ускорение операций с связками (КРИТИЧНО - 30сек → 5сек)
- [x] ✅ HTML-side кеширование названий (nameCache объект)
- [x] ✅ loadBindingNamesAsync() с setTimeout разброс
- [x] ✅ UI прогресс индикаторы (⏳ Обработка..., ✅ Готово, ❌ Ошибка)
- [x] ✅ getCachedVkGroupName/getCachedTgChatName fallback функции
- [ ] Убрать блокирующие API вызовы из критичных операций
- [ ] Оптимизировать таймауты (FAST_TIMEOUT для простых операций)

13) Фоновое обновление названий
- [x] ✅ loadSingleBindingName() для асинхронной загрузки
- [x] ✅ Разнесение запросов во времени (Math.random() * 1000)
- [x] ✅ localStorage кеш в браузере с проверкой актуальности
- [ ] Batch запросы для получения нескольких названий сразу

---

## 🧩 LINK CONVERSION — Правила и тест-кейсы (Server)

Правила:
- [screen_name|Title] → [Title](https://vk.com/screen_name)  
- [club123456|Название]/[public123456|Название]/[id123456|Имя] поддержаны
- [https://example.com|Текст] → [Текст](https://example.com)
- vk.com/wall-<id>_<postId> — отдельный линк (не заменяет Title)
- vk.cc/short — оставлять как есть (может вести на внешний сайт)
- Экранировать спецсимволы Markdown в Title

Тесты:
- [durov|Павел Дуров] → [Павел Дуров](https://vk.com/durov)
- [club123456|VK Dev] → [VK Dev](https://vk.com/club123456)  
- [daoqub|@daoqub] → [@daoqub](https://vk.com/daoqub)
- [durov|Павел Дуров (VK)] → [Павел Дуров (VK)](https://vk.com/durov)
- https://vk.com/wall-123_456 → остается как есть (standalone ссылка)
- [https://example.com|Сайт] → [Сайт](https://example.com)

---

## 🛠️ CODE SAMPLES (Cheat)

```javascript
// ИСПРАВЛЕННЫЙ VK URL парсинг
function extractVkGroupId(url) {
  if (!url) throw new Error('Пустая ссылка ВК');
  
  // Убираем query параметры (?from=groups, #hash)
  const cleanUrl = url.trim().toLowerCase().split('?')[0].split('#')[0];
  
  // Точные регулярки
  const patterns = [
    { regex: /vk\.com\/public(\d+)/i, prefix: '-' },  // public123
    { regex: /vk\.com\/club(\d+)/i, prefix: '-' },    // club123
    { regex: /vk\.com\/id(\d+)/i, prefix: '' },       // id123 (пользователи)
    { regex: /^-?(\d+)$/, prefix: function(match) {   // прямой ID
      return match[0].startsWith('-') ? '' : '-'; 
    }}
  ];

  for (const pattern of patterns) {
    const match = cleanUrl.match(pattern.regex);
    if (match) {
      const id = (typeof pattern.prefix === 'function') 
        ? pattern.prefix(match) + match[1]
        : pattern.prefix + match[1];
      
      logEvent("INFO", "vk_url_parsed", "client", `${url} → ${id}`);
      return id;
    }
  }
  
  throw new Error(`Не удалось извлечь ID: ${url}`);
}

// CLIENT: исправить передачу ID вместо URL
function checkNewPosts() {
  for (const binding of activeBindings) {
    const vkGroupId = extractVkGroupId(binding.vkGroupUrl);
    const posts = getVkPosts(vkGroupId);  // ← ID вместо URL!
  }
}

function getVkPosts(vkGroupId) {  // ← принимать ID
  const payload = {
    event: "get_vk_posts",
    license_key: license.key,
    vk_group_id: vkGroupId  // ← передать ID на сервер
  };
}

// SERVER: обновить структуру Bindings листа
function initializeServer() {
  createSheet("Bindings", [
    "Binding ID", "License Key", "User Email", 
    "Binding Name",        // НОВОЕ поле
    "Binding Description", // НОВОЕ поле
    "VK Group URL", "TG Chat ID", "Status", 
    "Created At", "Last Check", "Format Settings"
  ]);
}

// SERVER: поддержка новых полей в handleAddBinding
function handleAddBinding(payload, clientIp) {
  const { 
    license_key, 
    binding_name,        // НОВОЕ поле
    binding_description, // НОВОЕ поле
    vk_group_url, 
    tg_chat_id, 
    format_settings 
  } = payload;
  
  // Валидация обязательных полей
  if (!binding_name || binding_name.trim() === '') {
    return jsonResponse({
      success: false,
      error: "Binding name is required"
    }, 400);
  }
  
  // Сохранение с новыми полями
  bindingsSheet.appendRow([
    bindingId,
    license_key,
    license.email,
    binding_name.trim(),           // НОВОЕ поле
    binding_description.trim(),    // НОВОЕ поле
    vk_group_url,
    processedTgChatId,
    "active",
    new Date().toISOString(),
    new Date().toISOString(),
    formatSettingsString
  ]);
}

// HTML-side асинхронная загрузка названий
function loadBindingNamesAsync() {
  appState.bindings.forEach((binding, index) => {
    setTimeout(() => {
      google.script.run
        .withSuccessHandler(name => {
          if (name) document.getElementById(`vk-name-${binding.id}`).textContent = name;
        })
        .getCachedVkGroupName(binding.vkGroupUrl);
    }, index * 200); // 200мс между запросами
  });
}

// SIDEBAR интеграция
function openSidebar() {
  const html = HtmlService.createHtmlOutput(getMainPanelHtml())
    .setWidth(450)
    .setTitle("VK→Telegram Manager");
  
  SpreadsheetApp.getUi().showSidebar(html);
}
```

---

## 🧹 Чистка репозитория
- Удалить: все разрозненные TODO/README в to_fix, todo_server.gs, todo_clients.gs, todo_clients_permissions.gs, vk_link_markdown_todo.txt (перенесены сюда)
- Истина — этот единый файл: Unified TODO (корень)

---

## 📊 Статус реализации

### ✅ ВЫПОЛНЕНО (CLIENT):
- Тестирование связок (кнопка 🧪)
- Настройки форматирования в связках
- Глобальное отключение магазинов
- Автоматические триггеры
- Показ имен VK/TG групп в UI
- Детальное логирование
- Статистика и админ панель
- **UI для названий связок и описаний**
- **Collapse/Expand панель функциональность**
- **Асинхронная загрузка названий с кешированием**
- **Progress индикаторы для всех операций**
- **publishLastPost() функция реализована**

### ✅ ВЫПОЛНЕНО (SERVER):
- Получение названий VK/TG групп с кешированием
- Глобальные настройки (disable_all_stores)
- Расширенное логирование ошибок
- Валидация токенов при настройке
- Пакетные операции с названиями групп

### 🚨 КРИТИЧНО - НЕ РАБОТАЕТ:
- **VK URL parsing bug** - посты из неправильных групп!
- **Server.gs поддержка binding_name, binding_description**
- **Миграция структуры Bindings листа**
- **handlePublishLastPost обработчик**

### 🚧 В РАБОТЕ:
- Published-листы по названиям групп
- Первая синхронизация N постов
- Оптимизация производительности (30сек → 5сек)

### ⏳ ПЛАНИРУЕТСЯ:
- API рефакторинг и утилиты
- Строгие медиа-группы
- Comprehensive error handling
- Улучшенная обработка VK → TG ссылок