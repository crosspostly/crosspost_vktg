...
2. **CLIENT**: Для каждой активной связки запрашивает новые посты у сервера
3. **SERVER**: Получает посты из VK API с User Token
4. **CLIENT**: Сравнивает с последними ID в Published листах
5. **SERVER**: Отправляет новые посты в Telegram через Bot API
6. **CLIENT**: Записывает отправленные посты в Published листы

---

## 💾 Система хранения данных

### ❌ Удалён Properties кеш
**Проблема:** В UserProperties был кеш `LAST_POST_IDS`, что противоречило задаче полагаться только на листы сообщества.

**Решение:** Полный переход на Published листы:
```javascript
// ❌ Убрано:
function getLastPostIds() // из Properties
function saveLastPostIds(ids) // в Properties
...
- **Формат имени**: `Published_{BindingName}` (безопасное для Google Sheets)
...
- **Лист Bindings**: Связки VK→Telegram с настройками форматирования
...
## 📹 VK Видео: Прямые файлы (КРИТИЧНО #12)
...
## 🎭 Медиагруппы и смешанный контент (КРИТИЧНО #13)
